...
def test_sign_up_delegates_to_repo_create_user():
    ...
    ok = svc.sign_up("alice", "secret")

    assert ok is True
    repo.create_user.assert_called_once_with("alice", "secret")
...
def test_login_returns_none_if_verify_login_fails():
    ...
    svc = AppService(repo=repo, omdb=omdb)
    user = svc.login("alice", "badpass")

    assert user is None
    repo.verify_login.assert_called_once_with("alice", "badpass")
    repo.get_user_id.assert_not_called()
...
def test_login_returns_none_if_user_id_missing():
    ...
    repo.get_user_id.return_value = None

    svc = AppService(repo=repo, omdb=omdb)
    user = svc.login("alice", "secret")

    assert user is None
    repo.verify_login.assert_called_once_with("alice", "secret")
    repo.get_user_id.assert_called_once_with("alice")
...
def test_login_returns_session_user_on_success_and_strips_username():
    ...
    omdb = Mock()
    repo.verify_login.return_value = True
    repo.get_user_id.return_value = 42
